...
func BenchmarkMetricWrite(b *testing.B) {
	...
	tests := []struct {
		...
		metric		Metric
		expectedLength	int
	}{{testName: "value-1", metric: Metric{Name: "kube_pod_container_info", LabelKeys: []string{"container", "container_id", "image", "image_id", "namespace", "pod"}, LabelValues: []string{"container2", "docker://cd456", "k8s.gcr.io/hyperkube2", "docker://sha256:bbb", "ns2", "pod2"}, Value: float64(1)}, expectedLength: 168}, {testName: "value-35.7", metric: Metric{Name: "kube_pod_container_info", LabelKeys: []string{"container", "container_id", "image", "image_id", "namespace", "pod"}, LabelValues: []string{"container2", "docker://cd456", "k8s.gcr.io/hyperkube2", "docker://sha256:bbb", "ns2", "pod2"}, Value: float64(35.7)}, expectedLength: 171}}
