...
func TestStatefuleSetCollector(t *testing.T) {
	...
	const metadata = `
 	...
 	`
	cases := []generateMetricsTestCase{{Obj: &v1beta1.StatefulSet{ObjectMeta: metav1.ObjectMeta{Name: "statefulset1", CreationTimestamp: metav1.Time{Time: time.Unix(1500000000, 0)}, Namespace: "ns1", Labels: map[string]string{"app": "example1"}, Generation: 3}, Spec: v1beta1.StatefulSetSpec{Replicas: &statefulSet1Replicas, ServiceName: "statefulset1service"}, Status: v1beta1.StatefulSetStatus{ObservedGeneration: &statefulSet1ObservedGeneration, Replicas: 2, UpdateRevision: "ur1", CurrentRevision: "cr1"}}, Want: `
