...
func TestObjectsSameNameDifferentNamespaces(t *testing.T) {
	...
	genFunc := func(obj interface{}) []FamilyStringer {
		...
		metric := metrics.Metric{Name: "kube_service_info", LabelKeys: []string{"uid"}, LabelValues: []string{string(o.GetUID())}, Value: 1}
	...
	for _, id := range serviceIDS {
		s := v1.Service{ObjectMeta: metav1.ObjectMeta{Name: "service", Namespace: id, UID: types.UID(id)}}
		err := ms.Add(&s)
		if err != nil {
			t.Fatal(err)
		}
